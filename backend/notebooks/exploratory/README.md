...
| `quebec_tier1_envelope.py` | **The first end-to-end modeling notebook.** Pulls AgERA5 historical (3 years) + CanDCS-M6 mid-century (CanESM5 ssp245); computes per-crop indicators (GDD with crop-specific base T, growing-season days, mean growing-season T, annual precip); scores every Quebec staple (corn / soybean / spring wheat / canola) against the trapezoidal envelope; plots a 4×2 grid of present-vs-future suitability maps. | Copernicus CDS (`~/.cdsapirc`); PAVICS (no auth). |
| `quebec_wofost_smoke_test.py` | **The first Tier 2 (process model) run.** Pulls AgERA5 weather for a Montérégie point × 2018 (Apr–Oct), runs WOFOST 7.2 PP via `aigriculture.crop_models.wofost_runner` against PCSE's Grain_maize_201 cultivar, reports yield + phenology + yield-gap vs StatCan FCRS. | Copernicus CDS (`~/.cdsapirc`). |
